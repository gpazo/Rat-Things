...
The raw run API is not more privileged than a Thing. The same owner boundary, capability profile,
connection grants, approvals, validation, MicroVM isolation, idempotency, and durable evidence apply.
...
3. Use the returned request ID with the approval or response route; never invent a request ID.
...
There is no durable human-approval inbox in v1. Do not publish an unattended scheduled Thing that
can require user review unless the host will watch that run while its MicroVM is active; otherwise
use a policy/profile whose side effects are already bounded and whose approval behavior is suitable
for unattended work.
