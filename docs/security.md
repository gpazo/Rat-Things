...
| Artifact disclosure | Private S3, checksums, owner-hashed prefixes, authenticated owner check, short-lived presigned download URLs | API records still expose S3 coordinates. Add a safe external response projection, content classification, and audit/limits for URL issuance |
