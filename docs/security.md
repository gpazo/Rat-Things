...
| Artifact disclosure | Private S3, checksums, owner-hashed prefixes, authenticated owner check, bounded file catalogs, and unguessable time-bounded share URLs backed by one-minute S3 redirects | Run records still expose S3 coordinates. Add audit/rate limits for share issuance and access |
