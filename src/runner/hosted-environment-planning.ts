...
export function hostedCodexArguments(base: string[] | undefined, network: Extract<SessionLaunch['environment'], { type: 'openai_hosted' }>['network'], credentialProxy = false): string[] {
  ...
  return ['-c', 'default_permissions = "rat_managed"', '-c', `features.network_proxy = ${proxy}`, '-c',
    `permissions.rat_managed = { filesystem = { ":root" = "read", "/workspace" = "write", "/tmp" = "write" }, network = { enabled = ${network.access !== 'disabled'}, mode = "full", domains = { ${domains} }, proxy_url = "http://127.0.0.1:0", socks_url = "http://127.0.0.1:0", enable_socks5 = true, enable_socks5_udp = false, allow_local_binding = false, allow_upstream_proxy = ${credentialProxy} } }`,
    ...arguments_];
}
