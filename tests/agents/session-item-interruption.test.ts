...
describe('unfinished Items when execution ends', () => {
  it.each(['interrupted', 'failed', 'error', 'closed'] as const)('closes only root unfinished Items for %s', outcome => {
    ...
    expect(after.turns[0]!.items.map(item => 'status' in item ? item.status : null)).toEqual(['incomplete', 'incomplete', 'incomplete', 'incomplete', 'completed']);
    ...
    expect(events.filter(event => event.type === 'agent.session.turn.item.done')).toHaveLength(4);
  ...
  it('closes all active Items on harness loss without altering confirmed history', () => {
    ...
    for (const binding of after.turns) {
      ...
      expect(binding.items.map(item => 'status' in item ? item.status : null)).toEqual(['incomplete', 'incomplete', 'incomplete', 'incomplete', 'completed']);
    }
